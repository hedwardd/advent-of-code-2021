...
function getFirstStepWithFullFlash(energyLevels: number[][]): number {
  let flashCount = 0;
  ...
  while (true) {
    ...
    if (flashCount === energyLevels.length * energyLevels[0].length)
      return step;
  }
}
...
const firstStepWithFullFlash = getFirstStepWithFullFlash(startingEnergyLevels);

console.log(firstStepWithFullFlash);