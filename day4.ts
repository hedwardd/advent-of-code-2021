...
function getWinningScore(): number {
  ...
  for (let i = 0; i < callNumbers.length; i++) {
    ...
    const winningBoard = checkForWinningBoard(boards, numbersCalled);
    ...
    if (winningBoard !== null) {
      ...
      const sumOfNumbersNotCalled = getSumOfNumbersNotCalled(winningBoard, numbersCalled);
...
function checkForWinningBoard(boards: Board[], numbersCalled: Set<number>): Board | null {
  for (let board of boards) {
    const winningBoard = checkBoardForWinningBoard(board, numbersCalled);
    if (winningBoard !== null) {
      return winningBoard;
    }
  }
