...
function getMaxAltitudeFromStartingVelocity(startingVelocity: Velocity, targetArea: TargetArea): number | undefined {
  ...
  while (currentXPosition <= maxX && currentYPosition >= minY) {
    ...
    maxAltitude = Math.max(currentYPosition, maxAltitude);
    if (currentXPosition >= minX && currentXPosition <= maxX && currentYPosition <= maxY && currentYPosition >= minY) {
      hitsTargetArea = true;
    }
  }
  return hitsTargetArea ? maxAltitude : undefined;
}
...
let maxAltitude = -Number.MAX_VALUE;

// Bruce force
for (let xVelocity = 0; xVelocity <= targetArea[0][1]; xVelocity++) {
  for (let yVelocity = 0; yVelocity <= 1000; yVelocity++) {
    const altitude = getMaxAltitudeFromStartingVelocity([xVelocity, yVelocity], targetArea);
    if (altitude) {
